...
var AuthenticationContext = (function () {
    ...
    AuthenticationContext.prototype._openPopup = function (urlNavigate, title, popUpWidth, popUpHeight) {
        try {
            ...
            */
            ...
            var winOpen = /\bElectron/g.test(navigator.userAgent)?window.parent.open:window.open;
            var popupWindow = winOpen(urlNavigate, title, 'width=' + popUpWidth + ', height=' + popUpHeight + ', top=' + top + ', left=' + left);
    ...
    AuthenticationContext.prototype._loginPopup = function (urlNavigate, resource, callback) {
        ...
        var pollTimer = window.setInterval(function () {
            ...
            try {
                var location = popupWindow.location.href || popupWindow.location;
                if (location.indexOf(registeredRedirectUri) != -1) {
                    var hash = location.split('#')[1];
                    if (that.isAngular) {
                        that._onPopUpHashChanged(hash);
                    }
                    else {
                        that.handleWindowCallback(hash);
                    }
                    window.clearInterval(pollTimer);
                    that._loginInProgress = false;
                    that._acquireTokenInProgress = false;
